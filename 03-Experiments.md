...
如下圖所示，我們在鋁盤上的特定位置上打洞，並放置不同數量的鉬片或銅片用來調整 X-ray 照射到樣品上的強度。
...
另外在繞射強度分佈較廣的燒苗中，我們也可以使用自動調整 X-ray 照射強度的功能。
...
2. scan th 角度，找到反射強度最大的位置。

3. scan chi 角度，找到反射強度最大的位置。
...
假設我們使用 ca 指令計算 (0 1 1) 的馬達位置。發現 chi 為 -135˚，超出 chi : -95˚~5˚ 的範圍。
    ...
    Two Theta     Theta       Chi       Phi
      15.5480    7.7740 -135.0000  135.0000          <---- 這個 chi 的位置到不了
